...
//!
//! Provides [`RatifyLedgerStore`]: a full [`LedgerStore`] implementation that
//! adds working `approve_entry`, `reject_entry`, and `withdraw_entry` on top
//! of the standard append/list operations. The OSS [`AsgLedgerStore`] stubs
//! these methods with a commercial-feature error; `asd-pro` swaps in this
//! store instead.
//!
//! # Design
//!
//! [`RatifyLedgerStore`] wraps a [`Repository`] reference — the same substrate
//! as [`AsgLedgerStore`]. `asd-pro` constructs one directly from the opened
//! repository rather than going through `Engine`, so it bypasses the OSS
//! store entirely.
...
use agentstatedeveloper_core::{
    AsdError,
    error::Result,
    ledger::{ApprovalOutcome, LedgerStore, ReviewOutcome},
    paths,
    schema::LedgerEntry,
};
...
impl<'a> LedgerStore for RatifyLedgerStore<'a> {
    ...
    // Override list_entries here too so the superseded filter is consistent.
