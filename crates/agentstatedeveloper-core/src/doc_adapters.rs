...
pub fn adapt_document(path: &Path, content: &str) -> Option<Vec<SearchDoc>> {
    ...
    let is_manifest = matches!(name.as_str(),
        "package.swift" | "cargo.toml" | "pubspec.yaml" | "pubspec.yml" |
        "package.json" | "package-lock.json" | "podfile" | "gemfile" |
        "makefile" | "fastfile" | "appfile" | "deliverfile"
    );
    if is_manifest {
        let kind = if matches!(name.as_str(), "makefile" | "fastfile" | "podfile" | "gemfile") {
            DocKind::BuildScript
        } else {
            DocKind::Manifest
        };
...
pub fn is_doc_file(path: &Path) -> bool {
    ...
    matches!(name.as_str(),
        "package.swift" | "cargo.toml" | "pubspec.yaml" | "pubspec.yml" |
        "package.json" | "makefile" | "fastfile" | "podfile" | "gemfile"
    ) || matches!(ext.as_str(), "md" | "markdown" | "json" | "toml" | "yaml" | "yml" | "html" | "htm" | "css")
}
...
fn adapt_markdown(path: &Path, content: &str) -> Vec<SearchDoc> {
    ...
    if docs.is_empty() {
        let title = path.file_stem()
            ...
            .unwrap_or_default();
        docs.push(SearchDoc::new(DocKind::Markdown, path_str.as_ref(), None, title, content.chars().take(2000).collect::<String>()));
    }
    docs
}
...
fn adapt_json(path: &Path, content: &str) -> Vec<SearchDoc> {
    ...
    vec![SearchDoc::new(DocKind::Config, path_str.as_ref(), None, name, body)]
}
...
fn adapt_toml(path: &Path, content: &str) -> Vec<SearchDoc> {
    ...
    for line in content.lines() {
        ...
        if trimmed.starts_with('[') && trimmed.ends_with(']') {
            ...
            if !section_body.trim().is_empty() {
                docs.push(SearchDoc::new(DocKind::Manifest, path_str.as_ref(), Some(chunk_start), &current_section, section_body));
            }
            current_section = format!("{} {}", name, trimmed.trim_matches(|c| c == '[' || c == ']'));
            body_lines = Vec::new();
            chunk_start = line_no;
        } else if !trimmed.is_empty() && !trimmed.starts_with('#') {
    ...
    if docs.is_empty() {
        docs.push(SearchDoc::new(DocKind::Config, path_str.as_ref(), None, name, content.chars().take(1000).collect::<String>()));
    }
    docs
}
...
fn adapt_html(path: &Path, content: &str) -> Vec<SearchDoc> {
    ...
    vec![SearchDoc::new(DocKind::Html, path_str.as_ref(), None, name, text.chars().take(2000).collect::<String>())]
}
...
fn strip_html_tags(s: &str) -> String {
    ...
    while i < bytes.len() {
        ...
        match bytes[i] {
            b'<' => { in_tag = true; }
            b'>' => { in_tag = false; if !prev_space { out.push(' '); prev_space = true; } }
            _ if !in_tag => {
...
fn adapt_css(path: &Path, content: &str) -> Vec<SearchDoc> {
    ...
    for line in content.lines() {
        ...
        if t.ends_with('{') && !t.starts_with("/*") {
            ...
            if !sel.is_empty() { selectors.push(sel.to_string()); }
        }
        if t.starts_with("--") {
            let prop = t.split(':').next().unwrap_or("").trim();
            if !prop.is_empty() { custom_props.push(prop.to_string()); }
        }
    }
    let body = format!("{} {}", selectors.join(" "), custom_props.join(" "));
    if body.trim().is_empty() { return vec![]; }
    vec![SearchDoc::new(DocKind::Css, path_str.as_ref(), None, name, body.chars().take(2000).collect::<String>())]
}
...
fn adapt_manifest(path: &Path, content: &str, kind: DocKind) -> Vec<SearchDoc> {
    ...
    if kind == DocKind::BuildScript {
        ...
        if body.is_empty() {
            return vec![SearchDoc::new(kind, path_str.as_ref(), None, name, content.chars().take(500).collect::<String>())];
        }
