...
fn policy_list_returns_loaded_rules() {
    ...
    assert!(
        policies
            .iter()
            .any(|p| p["match_action"].as_str() == Some("asd.ledger.append.hazard")),
...
fn policy_evaluate_require_approval() {
    ...
    assert_eq!(
        v["status"].as_str().unwrap(),
        "awaiting-approval",
        "expected awaiting-approval; got {v}"
    );
...
fn policy_denies_ledger_append_at_write_time() {
    ...
    assert!(
        !o.status.success(),
        "expected ledger append to fail under deny policy"
    );
...
fn policy_allows_ledger_append_without_matching_rule() {
    ...
    assert!(
        o.status.success(),
        "expected append to succeed; stderr: {}",
        String::from_utf8_lossy(&o.stderr)
    );
    ...
    assert!(
        v["entry_id"].as_str().is_some(),
        "entry_id missing from response"
    );
}
