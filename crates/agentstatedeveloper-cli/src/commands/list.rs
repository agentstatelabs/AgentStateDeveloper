...
pub fn run(cfg: &Config, args: ListArgs) -> Result<()> {
    ...
    match args.cmd {
            ...
            if let Some(cat) = &category {
                decls.retain(|d| {
                    d.declared.iter().any(|e| e.effect.as_str() == cat.as_str())
                        || d.transitive
                            .iter()
                            .any(|t| t.effect.as_str() == cat.as_str())
                });
            }
        ...
        ListCmd::Stats => {
            ...
            let mut by_category: Vec<_> = by_category
                .into_iter()
                .map(|(k, v)| json!({"category": k, "count": v}))
                .collect();
            by_category.sort_by(|a, b| b["count"].as_u64().cmp(&a["count"].as_u64()));
            let mut by_verification: Vec<_> = by_verification
                .into_iter()
                .map(|(k, v)| json!({"status": k, "count": v}))
                .collect();
            ...
            let mut ledger_by_kind: Vec<_> = ledger_by_kind
                .into_iter()
                .map(|(k, v)| json!({"kind": k, "count": v}))
                .collect();
