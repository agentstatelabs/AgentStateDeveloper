...
| t-004c | **Import-alias resolution**: key routers by `(file, var)` identity and resolve `include_router(alias)` back to the aliased router's source module+name via the existing `parse_imports`/`ImportBinding` machinery, so the decorator's receiver connects to the mount edge. This is the actual closer for real-world FastAPI apps (Financial's remaining ~65/68) | Team | 1 |
