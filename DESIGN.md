...
|------|-------------|------|
| t-001 | **Federation setup doc** ✅: `docs/FEDERATION.md` — index each repo → registry (`asd repo add/list`) → `asd repo edges/impact`; the hub's `code_cross_repo_edges`/`code_impact`; distinct-`repo_id`, freshness, drift, and independent-sessions notes. Linked from README | 1 ✅ |
| t-002 | **Public/private crate boundary** (decided): OSS-public = `core` + 9 language crates + `adapters` + `conformance` + `cli` + `mcp`; private = `audit-pro` + `ratify` + `pro` (the `asd-pro*` binaries). Dependency direction verified clean: OSS never imports pro; pro depends on OSS. The private **`AgentStateDeveloper-Enterprise`** repo already has this structure (crates: audit-pro/ratify/asd-pro; OSS via git/tag deps) — but pins OSS at **v1.0.13** while the live pro crates in the OSS tree track v1.1.23 (drift to reconcile) | 1 |
| t-003 | **Finish the drifted split** ✅: synced live pro crates → Enterprise repo, fixed its `[workspace.package]` (edition 2024, `license`), pointed `members` at `agentstatedeveloper-pro`, pinned OSS **v1.2.0** (fresh tag cut at HEAD — the old v1.1.23 was 74 commits stale); Enterprise builds clean against OSS@v1.2.0. Then removed `audit-pro`/`ratify`/`pro` from the OSS workspace (members + deps + dirs) — **OSS now builds core-only** (`cargo check --workspace` green). Public repo is commercial-crate-free | 1 ✅ |
| t-004 | **`asd-mcp` CWD walk-up resolution** ✅: shared `core::registry::find_db_upwards()` (CLI deduped onto it); `asd-mcp` resolves `ASD_DB` > CWD walk-up > registry-active, so each session spawned in its project dir auto-isolates. **Verified: two concurrent sessions in two projects each resolved their own db.** FEDERATION.md updated | 2 ✅ |
| t-005 | **`asd mcp install --project`** ✅: writes a project-local `.mcp.json` (standard `mcpServers.asd` shape) pinned to the project's absolute db — the explicit per-project isolation path alongside t-004's automatic walk-up. Reuses the JSON upsert helpers; verified | 2 ✅ |
