...
| t-003 | **Finish the drifted split**: sync the live pro crates (OSS tree → Enterprise repo), bump Enterprise's OSS git-dep tag to current, verify Enterprise builds; then **remove `audit-pro`/`ratify`/`pro` from the OSS workspace** (members + `workspace.dependencies` + directories) so the public repo is core-only, and verify it builds standalone. Cross-repo + LAN pushes are operator-run | 1 |
